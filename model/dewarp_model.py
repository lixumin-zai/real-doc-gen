...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, mask):
        ...
        dim_t = torch.arange(self.num_pos_feats, dtype=torch.float32).to(mask)
...
def safe_add_sbc(a, b):
    """
    Safely adds two tensors with shape (Sequence, Batch, Channels)
    for PNNX/NCNN compatibility.
    """
    # (S, B, C) -> (B, S, C)
    ...
    # Add in (B, S, C) format
    result_perm = a_perm + b_perm
    
    # (B, S, C) -> (S, B, C)
    result = result_perm.permute(1, 0, 2)
    return result
...
class attnLayer(nn.Module):
    ...
    def with_pos_embed(self, tensor, pos: Optional[Tensor]):
        if pos is None:
            ...
            # 原始操作: tensor + pos
            # PNNX 不喜欢的操作，因为 Batch 在中间维度
            
            # 新操作:
            # 1. (Seq, Batch, C) -> (Batch, Seq, C)
            tensor_permuted = tensor.permute(1, 0, 2)
            pos_permuted = pos.permute(1, 0, 2)
            
            # 2. 在 Batch 是第一维的情况下进行加法
            result_permuted = tensor_permuted + pos_permuted
            
            # 3. (Batch, Seq, C) -> (Seq, Batch, C)
            result = result_permuted.permute(1, 0, 2)
    ...
    def forward_post(self, tgt, memory_list, tgt_mask=None, memory_mask=None,
        ...
        # 使用 safe_add_sbc 替换 '+'
        tgt = safe_add_sbc(tgt, self.dropout1(tgt2))
        tgt = self.norm1(tgt)

        # 2. Cross-Attention
        # 恢复原始的 zip 循环，因为它本身没问题
        for memory, multihead_attn, norm2, dropout2, m_pos in zip(memory_list, self.multihead_attn_list, self.norm2_list, self.dropout2_list, memory_pos):
            ...
            # 使用 safe_add_sbc 替换 '+'
            tgt = safe_add_sbc(tgt, dropout2(tgt2))
            tgt = norm2(tgt)
        ...
        # 使用 safe_add_sbc 替换 '+'
        tgt = safe_add_sbc(tgt, self.dropout3(tgt2))
        tgt = self.norm3(tgt)
        return tgt
...
class TransDecoder(nn.Module):
    ...
    def forward(self, imgf, query_embed):
        pos = self.position_embedding(torch.ones(imgf.shape[0], imgf.shape[2], imgf.shape[3], device=imgf.device, dtype=imgf.dtype))  # torch.Size([1, 128, 36, 36])
        pos = pos.to(imgf)
        bs, c, h, w = imgf.shape
...
class TransEncoder(nn.Module):
    ...
    def forward(self, imgf):
        ...
        # imgf = imgf.flatten(2).permute(2, 0, 1)  # -> [H*W, B, C]
        # pos = pos.flatten(2).permute(2, 0, 1)    # -> [H*W, B, C]

        # for layer in self.layers:
        #     imgf = layer(imgf, [imgf], pos=pos, memory_pos=[pos, pos])
            
        # imgf = imgf.permute(1, 2, 0).reshape(bs, c, h, w) # -> [B, C, H, W]
        # return imgf
...
class GeoTr(nn.Module):
    def __init__(self):
        ...
        # self.flow_upsampler = FlowUpsampleModule()
    ...
    def forward(self, image1, coords1, return_features=False):
        ...
        query_embed0 = self.query_embed.weight.unsqueeze(1).expand(-1, fmap3.size(0), -1)
